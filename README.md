<div align="center">

  <!-- Гифка (Матрица) -->
  <img src="https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExZ3I4cHhoeWRtYzhocTF2d2dseW42c25memRjZjJsaGg3NjI0ZWxuciZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/1yvoDVJQsTfHi/giphy.gif" width="100%" height="220" style="object-fit: cover; border-radius: 10px;" />

  # 👋 Привет, я Макар Лебедев
  
  ### 💻 Frontend-разработчик
  
  <!-- Анимированная строка -->
  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=24&pause=1000&color=3B82F6&center=true&vCenter=true&width=500&lines=HTML+%7C+CSS+%7C+JavaScript;Building+Modern+Websites;Code+is+Art" alt="Typing SVG" />
  </a>

</div>

---

### 🚀 Обо мне

Я начинающий разработчик, сейчас фокусируюсь на создании удобных и красивых интерфейсов.

- 🔭 Студент **ITtop Academy**.
- 🌱 Мой основной стек: **HTML5, CSS3, JavaScript**.
- 🎯 Цель: Стать Fullstack-разработчиком и запустить свой крупный сервис.
- ⚡ **Философия:** Писать чистый код и постоянно учиться новому.

---

### 🛠️ Технологии и Инструменты

<div align="center">
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white" />
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white" />
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" />
  <img src="https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visual%20studio%20code&logoColor=white" />
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" />
</div>

---

### 📂 Мои Проекты

| Проект | Описание | Стек |
| :--- | :--- | :--- |
| **🏗️ [Название проекта]** | Краткое описание (например: Лендинг для кофейни). | `HTML` `CSS` |
| **📱 [Название проекта]** | (Здесь будет ваш будущий проект). | `JS` |

---

### 📧 Как со мной связаться

✉️ **Предпочитаемый способ:** Telegram.

<div align="center">

| Способ связи | Контакт | Время ответа ⏱️ |
| :--- | :--- | :--- |
| <img src="https://img.shields.io/badge/Telegram-2CA5E0?style=flat&logo=telegram&logoColor=white"/> | **@DeadRead7** | 1-2 часа |
| <img src="https://img.shields.io/badge/Discord-5865F2?style=flat&logo=discord&logoColor=white"/> | **nooptimist** | ~ 1 час |
| <img src="https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white"/> | **akkMakara@mail.ru** | ~ 24 часа |

</div>

---

### 🛫 Мой бэкграунд (Хобби)

До того как прийти в IT, я увлекался инженерией и спортом. Этот опыт научил меня дисциплине и работе руками.

| Период | Увлечение | Опыт |
| :--- | :--- | :--- |
| **2019 - 2026** | 
